...
// ---- ExamForm: 최상단 컴포넌트로 분리 (내부 정의 시 base-ui Select 렌더 이슈 발생) ----
function ExamForm({
  value,
  onChange,
  subjects,
        ...
        >
          <SelectTrigger className="w-full">
            <span className={value.subjectId ? undefined : "text-muted-foreground"}>
              {value.subjectId
                ? (subjects.find((s) => s.id === value.subjectId)?.name ?? "과목 선택")
                : "과목 선택"}
            </span>
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          ...
          <Select
            ...
            onValueChange={(v) => onChange({ ...value, examType: v as ExamType })}
          >
            <SelectTrigger className="w-full">
            ...
            editingId === exam.id ? (
              <div key={exam.id} className="bg-card border-border rounded-xl border p-5 shadow-sm space-y-3">
                <ExamForm
                  ...
                  subjects={subjects}
                  calOpen={editCalendarOpen}
                  setCalOpen={setEditCalendarOpen}
                />
                <div className="flex gap-2">
              ...
              <div key={exam.id} className="bg-card border-border flex items-center justify-between rounded-xl border p-5 shadow-sm">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    onClick={() => handleDelete(exam.id, `${exam.subjectName ?? "과목 미지정"} ${EXAM_TYPE_LABEL[exam.examType]}`)}
