...
import { createSubject, updateSubject, deleteSubject, getSubjects } from "@/lib/actions/subjects";
...
export function SubjectsClient({ initialSubjects }: { initialSubjects: Subject[] }) {
  ...
  function refresh() {
    getSubjects().then(setSubjects).catch(() => setActionError("목록 갱신에 실패했습니다. 새로고침해주세요."));
    router.refresh();
  }
            ...
            editingId === s.id ? (
              ...
              <div key={s.id} className="bg-card border-border flex items-center justify-between rounded-xl border p-5 shadow-sm">
                ...
                <div className="flex items-center gap-1">
                  <button type="button" onClick={() => handleEditStart(s)} className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
                    <Pencil className="h-4 w-4" />
                  </button>
                  <button type="button" onClick={() => handleDelete(s.id, s.name)} className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-red-50 hover:text-red-600">
